...
namespace WanderingCity.Tests
{
    public sealed class GameplayTests
    {
        ...
        [UnityTest] public IEnumerator EveryRewardHasAReachableInteractionPosition()
        {
            ...
            foreach (var item in game.World.Interactions)
            {
                ...
                for (int i = 0; i < 16; i++) { float angle = i * Mathf.PI / 8; Vector3 probe = item.transform.position + new Vector3(Mathf.Cos(angle), 0, Mathf.Sin(angle)) * 2.6f; if (!NavMesh.SamplePosition(probe, out var nav, 2, NavMesh.AllAreas)) continue;
                    var path = new NavMeshPath(); if (NavMesh.CalculatePath(Vector3.zero, nav.position, NavMesh.AllAreas, path) && path.status == NavMeshPathStatus.PathComplete && Vector3.Distance(nav.position + Vector3.up, item.transform.position) < 3.6f && CombatVisibility.Clear(nav.position + Vector3.up, item.transform.position)) { found = true; break; } }
                if (!found) missing.Add(item.Id);
            }
        ...
        {
            var enemy = game.World.Enemies[0]; enemy.Home = new Vector3(0, 0, 30); enemy.Agent.Warp(enemy.Home); Teleport(enemy.Home + Vector3.back * 5); yield return new WaitForSeconds(.4f); Assert.AreEqual(EnemyAction.Chase, enemy.Action);
        ...
        {
            var enemy = game.World.Enemies[0]; enemy.Agent.enabled = false; enemy.transform.position = new Vector3(0, 0, 22); Teleport(new Vector3(0, 0, 20)); yield return null;
            Assert.IsTrue(game.Player.StartAttack()); game.Player.Strike(); game.Player.Strike(); Assert.AreEqual(78, enemy.Hp);
            game.Player.RestorePosition(); var wall = GameObject.CreatePrimitive(PrimitiveType.Cube); wall.transform.position = new Vector3(0, 1, 21); wall.transform.localScale = new Vector3(4, 3, .25f); Physics.SyncTransforms();
        ...
        [UnityTest] public IEnumerator DeathRespawnPreservesInventoryAndRewards()
        {
            ...
            Assert.AreEqual(100, game.State.hp); Assert.AreEqual(4, game.State.Count("ore")); Assert.Contains("chest-forest", game.State.claimed); Assert.Less(Vector3.Distance(game.Player.transform.position, Vector3.up), 1);
        }
        ...
        [UnityTest] public IEnumerator BuildingsBlockPlayerAndRestoreAsColliders()
        {
            ...
            Assert.IsTrue(Physics.Linecast(new Vector3(-6, 1, 0), new Vector3(-6, 1, 3), 1 << 0)); Assert.IsTrue(game.Save()); Assert.IsTrue(game.Saves.Load(out var loaded, out _)); Assert.AreEqual(2, loaded.buildings.Count);
        }
        [UnityTest] public IEnumerator ControllerStopsAtWalls()
        {
            Teleport(new Vector3(0, .1f, 20)); var wall = GameObject.CreatePrimitive(PrimitiveType.Cube); wall.transform.position = new Vector3(0, 1, 22); wall.transform.localScale = new Vector3(10, 3, .3f); Physics.SyncTransforms();
