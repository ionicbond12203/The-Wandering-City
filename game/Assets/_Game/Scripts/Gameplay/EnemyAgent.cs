...
namespace WanderingCity
{
    ...
    public sealed class EnemyAgent : MonoBehaviour
    {
        ...
        public void ResetEncounter() { Hp = Session.Balance.enemyHealth; Action = EnemyAction.Patrol; phase = cooldown = 0; if (gameObject.activeInHierarchy && Agent.isOnNavMesh) { Agent.Warp(Home); Agent.isStopped = false; Agent.ResetPath(); } }
        void Update()
        {
            ...
            if (Action == EnemyAction.Attack)
            {
                ...
                if (!struck && phase >= .8f) { struck = true; if (distance < 2.5f && reachable && CombatVisibility.Clear(transform.position + Vector3.up, player.transform.position + Vector3.up)) player.Damage(Session.Balance.enemyDamage); }
