...
namespace WanderingCity
{
    ...
    public sealed class OrbitCamera : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            if (Session.InputReady && Mouse.current != null) { Vector2 delta = Mouse.current.delta.ReadValue(); Yaw += delta.x * .12f; Pitch = Mathf.Clamp(Pitch - delta.y * .1f, -25, 65); distance = Mathf.Clamp(distance - Mouse.current.scroll.ReadValue().y * .005f, 2.5f, 8f); }
